...
function pdo(): PDO {
  // Railway reference (prefer)
  $url = getenv('DATABASE_URL') ?: getenv('DATABASE_URL_PUBLIC_URL');

  // Fallback to PG* vars (como en tu captura)
  if (!$url) {
  ...
  $p = parse_url($url);
  if (!$p) throw new RuntimeException("DATABASE_URL inválida");
  ...
  // Si es host público proxy, forzamos ssl
  $needsSsl = str_contains($host, '.proxy.rlwy.net');
  $dsn = "pgsql:host=$host;port=$port;dbname=$db" . ($needsSsl ? ";sslmode=require" : "");
